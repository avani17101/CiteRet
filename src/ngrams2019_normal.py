...
model = torch.load('bert2019_full_dset.pth')
path = '/ssd_scratch/cvit/dhawals1939/'
filename = path+'test_docs_2019_ngrams_normal_0.pkl'
new_test_docs = {}
...
#         corpus_embedding = model.encode(corpus)
    ...
    if j % 10 == 0:
        filename = path+f'test_docs_2019_ngrams_normal_{j}.pkl'
        print(filename)
...
filename = path+f'test_docs_2019_ngrams_normal_{j+1}.pkl'
print(new_test_docs)
print(filename)
with open(filename, 'wb') as handle:
    pickle.dump(new_test_docs, handle, protocol=pickle.HIGHEST_PROTOCOL)